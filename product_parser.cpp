...
{
  Product* book=new Book(prodName_, author_, isbn_, price_, qty_);
  return book;
}
...
{
  Product* clothing=new Clothing(prodName_, size_, brand_, price_, qty_);
  return clothing;
}
...
{
  Product* movie=new Movie(prodName_, genre_, rating_, price_, qty_);
  return movie;
}
